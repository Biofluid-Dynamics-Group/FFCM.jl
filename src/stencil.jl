...
`j_i = round(Y_i · inv_Δx)` (`RoundNearestTiesToEven`, the cuFCM `my_rint`
convention; paper §5 Table 2 calibration). For `k ∈ 1:M_G` it writes:

- `gauss_*[k] = inv_norm · exp(−x² · inv_2Σ²)` — the separable 1-D Gaussian
  weight, with `x = (j_i − ⌊M_G/2⌋ + (k−1))·Δx − Y_i` the unwrapped stencil
...
- `ind_*[k]` — the periodic-wrapped 1-based grid index `mod(g_i, M_i) + 1`.

Preconditions (caller-guaranteed, so the loop is `@inbounds`): the nine
scratch vectors have length `M_G`; `ngdh = M_G ÷ 2`; `num_grid_points` holds
