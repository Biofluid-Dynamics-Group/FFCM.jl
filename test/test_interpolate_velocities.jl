...
@testset "Interpolation is the discrete adjoint of spreading (J = Δx³ Sᵀ)" begin
    ...
    # spreading operator J†, scaled by the trapezoidal weight Δx³
    # (paper `outline.tex:324`). Concretely, for any grid field `u` and
    # any particle/component unit vector `e_{n,c}`:
    #
    #   ⟨interpolate(u), e_{n,c}⟩ = Δx³ · ⟨u, spread(e_{n,c})⟩.
    #
    # `spread(e_{n,c})` puts a unit force on particle `n` in direction `c`,
    # so force_grid component `c` equals Δ̃_n(x_g) and the others vanish;
    ...
    for T in (Float32, Float64)
        ...
        for n in 1:N, c in 1:3
            ...
            adjoint_value = Δx³ * sum(u_comp[c] .* f_comp[c])
...
@testset "Interpolated velocities are returned in original particle order" begin
    ...
    for T in (Float32, Float64)
        ...
        deterministic_field!(config.velocity_grid)
        ...
        deterministic_field!(ref.velocity_grid)
        Vn = zeros(T, 3, 1)
        rtol = sqrt(eps(T))
        atol = _interp_atol(T)
        for n in 1:N
...
@testset "Single-particle interpolation matches the closed-form modified kernel" begin
    ...
    for T in (Float32, Float64)
        ...
        for iz in 1:M, iy in 1:M, ix in 1:M
            ...
            ref = (
                ref[1] + ux[ix, iy, iz] * Δ̃ * Δx³,
                ref[2] + uy[ix, iy, iz] * Δ̃ * Δx³,
                ref[3] + uz[ix, iy, iz] * Δ̃ * Δx³,
            )
        end
        rtol = sqrt(eps(T))
        atol = _interp_atol(T)
...
@testset "Interpolating a constant flow returns that constant (rigid-body limit)" begin
    ...
    for T in (Float32, Float64)
        ...
        # sqrt(eps(T)) with the same (M_G = 16, Σ/Δx) regime, so the kernel
...
@testset "Translating particle and flow by one grid cell leaves the velocity unchanged" begin
    ...
    # particle by Δx along axis i and cyclically shifting the flow by one
    ...
    for T in (Float32, Float64)
        ...
        # +1 shift of the stencil indices when the particle moves by +Δx.
        ux .= circshift(base[1], (1, 0, 0))
        uy .= circshift(base[2], (1, 0, 0))
        uz .= circshift(base[3], (1, 0, 0))
        config.Y_sorted[:, 1] .= (Y_a[1] + Δx, Y_a[2], Y_a[3])
...
@testset "Σ = σ collapses interpolation to the standard-FCM Gaussian average" begin
    # Σ_over_σ = 1 ⇒ pdmag = 0 ⇒ a₀ = 1, a₂ = 0 ⇒ Δ̃_n = Δ_n. The particle
    ...
    for T in (Float32, Float64)
        ...
        for iz in 1:M, iy in 1:M, ix in 1:M
            ...
            Δ = (T(2) * T(π) * σ^2)^(-T(3) / T(2)) * exp(-r² / (T(2) * σ^2))
            ref = (
                ref[1] + ux[ix, iy, iz] * Δ * Δx³,
                ref[2] + uy[ix, iy, iz] * Δ * Δx³,
                ref[3] + uz[ix, iy, iz] * Δ * Δx³,
            )
        end
        rtol = sqrt(eps(T))
        atol = _interp_atol(T)
...
@testset "Assembled mobility M^VF is symmetric (spread/solve/interpolate adjoint)" begin
    # M^VF = Δx³·(SP)ᵀ·L⁻¹·(SP) is symmetric positive-definite because L⁻¹
